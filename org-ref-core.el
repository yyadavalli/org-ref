...
(defcustom org-ref-bibliography-notes
  nil
  "Filename to put all your notes about an entry in the default bibliography.
Used by backends that append all notes as entries in a single
file.
...
(defcustom org-ref-notes-directory
  nil
  "Directory to put all your notes about an entry in the default bibliography.
...
`org-ref-get-pdf-filename'. Alternative value is
...
(defun org-ref-enable-colorized-links ()
  "Colorize org-ref links.

The buffer might have to be reverted for it to take effect."
  (when org-ref-colorize-links
    (add-hook
     'org-mode-hook
     (lambda ()
       (advice-add 'org-activate-bracket-links
                   :after #'org-ref-make-org-link-cite-key-visible)
...
(defun org-ref-open-bibliography-no-org (link)
  "Open bibliography in LINK, when you are not in `org-mode'.
...
we open it, otherwise prompt for which one to open."
  (let ((bibfiles (split-string link ",")))
    (find-file (if (= 1 (length bibfiles))
                   (car bibfiles)
                 (completing-read "Bib file: " bibfiles nil t)))))
...
(defun org-ref-open-bibliography (link-string)
  "The click function for a bibliography LINK-STRING."
    ...
    (let* ((bibfile) ;; object is the link you clicked on
      ...
      (let (key-beginning key-end)
        ...
        (setq bibfile (org-ref-strip-string
      	...
      	;; open file on click. I use `or' because org-ref-find-bibfile returns
      	;; nil if the file doesn't exist, and clicking should open the file in
      	;; that case.
...
Bibliograhpy given by KEYWORD is formatted with description DESC in FORMAT"
  (cond
...
(defun org-bibliography-complete-link ()
...
(defun org-ref-bibliography-face-fn (path)
  "Return face for a bibliography link at PATH.
...
Bibliography given with KEYWORD is formatted with description DESC in FORMAT"
  (cond
   ((eq format 'org) (org-ref-get-org-bibliography))
   ((eq format 'ascii) (org-ref-get-ascii-bibliography))
   ((eq format 'odt) (org-ref-get-odt-bibliography))
...
(defun org-ref-follow-addbibresource (link-string)
  "On clicking add bibliography resource give by LINK-STRING."
  ;; get link-string boundaries. we have to go to the
  ;; beginning of the line, and then search forward
  (let* ((bibfile)
           ...
           (list-of-tables
            (org-element-map (org-element-parse-buffer 'element) 'table
                             (lambda (table)
                               ...
                               (save-excursion
                                 (when (goto-char (org-element-property :begin table))
                                   (not (or (org-in-commented-heading-p)
                                            (-intersection
...
(defun org-ref-label-face-fn (label)
  "Return a face for the label link, LABEL."
  (save-match-data
    (cond
     ((or (not org-ref-show-broken-links)
...
(defun org-ref-ref-help-echo (window object position)
  "A help-echo function for ref links in WINDOW of the type OBJECT at POSITION."
  (save-excursion
    (goto-char position)
    (let ((s (org-ref-link-message)))
      (with-temp-buffer
...
The reference given by KEYWORD is formatted with description DESC in FORMAT"
  (cond
   ((eq format 'html)
...
(defun org-ref-ref-face-fn (label)
  "Return a face for a ref link with label, LABEL."
  (save-match-data
    (cond
     ((or (not org-ref-show-broken-links)
...
(org-ref-link-set-parameters "pageref"
  ...
  :complete #'org-ref-pageref-complete-link
  :help-echo #'org-ref-ref-help-echo)


(defun org-ref-pageref-complete-link ()
  "Completion function for ref links."
  (let ((label))
    (setq label (completing-read "label: " (org-ref-get-labels)))
    (format "ref:%s" label)))
...
(defun org-ref-eqref-export (keyword desc format)
  "Equation Export function.
The equation with KEYWORD is formatted with the description DESC in FORMAT."
  (cond
...
The nameref in KEYWORD is formatted with the description DESC in FORMAT."
  (cond
...
(defun org-ref-cref-export (keyword desc format)
  "Link export function for `cref' links.
KEYWORD is used as the keyword for a link.
DESC and FORMAT are ignored.
See https://www.ctan.org/tex-archive/macros/latex/contrib/cleveref"
  (format "\\cref{%s}" keyword))
      ...
      ((eq format 'ascii)
       (concat "["
               (mapconcat (lambda (key) (format "%s" key))
                          (org-ref-split-and-strip-string keyword) ",") "]"))
      ((eq format 'html)
       (mapconcat (lambda (key) (format org-ref-ref-html key key))
                  (org-ref-split-and-strip-string keyword) ","))
      ((eq format 'latex)
          ...
          (let ((bibtex-completion-bibliography (org-ref-find-bibliography)))
            (-every?
             ...
             (mapcar
              (lambda (key)
                (assoc "=key="
                       (bibtex-parse-entry (bibtex-search-entry key t))))
              (split-string keys ",")))))
      'org-ref-cite-face)
     (t
      'font-lock-warning-face))))
...
PATH is required for the org-link, but it does nothing here."
  ...
  (let ((*index-links* '())
    ...
    (erase-buffer)
    (insert "#+TITLE: Index\n\n")
    (cl-dolist (letter *initial-letters*)
      (insert (format "* %s\n" (upcase letter)))
...
(defun org-ref-open-in-browser ()
  "Open the bibtex entry at point in a browser using the url or doi field."
  (interactive)
  (save-excursion
    (bibtex-beginning-of-entry)
    (catch 'done
...
(defun org-ref-bad-cite-candidates ()
  "Return a list of bad cite references.
The returned list contains conses (key . marker) where key does not exist in the
known bibliography files, and marker points to the key."
...
(defun org-ref-bad-ref-candidates ()
  "Return a list of conses (ref . marker).
Where ref is a ref link that does not point to anything (i.e. a label)."
...
(defun org-ref-bad-file-link-candidates ()
  "Return list of conses (link . marker).
where the file in the link does not exist."
  (let* ((bad-files '()))
    (org-element-map (org-element-parse-buffer) 'link
                     (lambda (link)
...
(defun org-ref-downcase-bibtex-entry ()
  ...
  (let* ((entry (bibtex-parse-entry))
    ...
    (setq entry-fields (remove "=type=" entry-fields))
    (bibtex-kill-entry)
    (insert (concat "@" (downcase type) "{" key ",\n"
                    (mapconcat
                     (lambda (field)
                       (format "%s = %s,"
                               (downcase field)
...
If ARG is true, bibtex entry is cleaned.
...
move to the beginning of the previous cite link after this one."
  ...
  (let ((cps (org-ref-parse-cite))
    ...
    (cond
     ...
     (t
      (setq index (cl-loop for i from 0
                           for (k s e) in cps
                           if (and (>= p s)
                                   (<= p e))
                           return i))
            ...
            (cond
             ...
             ((string= type "label")
              (let ((count (org-ref-count-labels
                ...
                (message (concat (number-to-string count)
                                 " occurrence"
                                 (when (or (= count 0)
                                           (> count 1))
                                   "s")))))
    ...
    (cond
     ...
     ((and (-contains? org-ref-cite-types type)
           (< (point) (+ (org-element-property :begin cite)
                         (length type) 1)))
      (setq begin (org-element-property :begin cite)
            end (org-element-property :end cite)
            ...
            new-cite (concat (when bracket-p "[[")
                             type
                             ":"
  ...
  (save-excursion
    (let* ((cite (org-element-context))
      ...
      (let* ((cite (org-element-context))
        ...
        (cl-dolist (nkey (reverse new-keys))
