...
generated by `org-ref-reftex-format-citation'."
  ...
  (let ((bib
    ...
    (switch-to-buffer-other-window
     (format "%s-bibliography" (buffer-file-name)))
    (erase-buffer)
    (insert bib)
    (org-mode)))
...
(defun org-ref-get-bibtex-entry-html (key)
  ...
  (let ((output))
    ...
    (setq output (org-ref-escape-html output))
    (format "<li><a id=\"%s\">[%s]</a> %s</li>"
            key key output)))

(defun org-ref-escape-html (output)
    ;; unescape the &
    ...
    (setq output (replace-regexp-in-string "<b></b>," "" output))
...
(defun org-ref-get-bibtex-entry-md (key)
  ...
  (format "<a id=\"%s\"></a>[%s] %s%s [↩](#%s)"
	  key key
	  (org-ref-escape-html (org-ref-get-bibtex-entry-citation key))
	  ""
