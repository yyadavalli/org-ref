...
;; multiple DOIs are found, you will get an ivy prompt to choose what you want
;; to add. You can add new patterns to `org-ref-doi-regexps'.
...
We scrape DOIs from the URL first. If there is one, we add it using ACTION. If
there is more than one, we offer an ivy selection. If no DOI is found, we create
a misc entry, with a prompt for a key."
...
A doi will be either doi:10.xxx  or 10.xxx. It is handled using ACTION."
  (if (and (buffer-file-name)
           (f-ext? (buffer-file-name) "bib"))
...
;;;###autoload
(defun org-ref-url-dnd-debug (url)
  "Drag-n-drop function to debug a URL."
  (interactive "e")
  (org-ref-url-debug-url (cadr (car (last url)))))
...
not perfect, and some hits are not actually DOIs."
  (interactive "e")
  (org-ref-url-add-all-doi-entries (cadr (car (last url)))))
...
;;;###autoload
(defun org-ref-url-dnd-first (url)
  "Drag-n-drop function to download the first DOI in a URL."
  (interactive "e")
  (org-ref-url-add-first-doi-entry (cadr (car (last url)))))
